...
class GenerateInterfaceCommand extends Command
{
    ...
    protected static $defaultName = 'generate:react:interface';
    ...
    {
        $this->setDescription('Generates react interface')
            ->setHelp('Eg: bin/console generate:react:interface TestInterface TestBundle')
            ->addArgument(self::KEY_FILE, InputArgument::REQUIRED, 'React interface file')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $inputOutput->title('Generate react interface');

        /** @var string $fileName */
        $fileName = $input->getArgument(self::KEY_FILE);
        if (!strpos($fileName, 'Interface')) {
            $fileName .= 'Interface';
        }
        ...
        $stub->setStubPath(__DIR__ .
            '/../../Resources/stub/Generate/Resources/assets/Interface/SampleInterface.tsx.stub');
